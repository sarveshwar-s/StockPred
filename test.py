...
db = mysql.connector.connect(
    host="stock.crne5vznig3b.ap-south-1.rds.amazonaws.com", user="admin", passwd="YOUR_DB_PASSWORD", database="trading"
)
...
def show_financial_state(fincompanyname):
    ...
    finance_quarter_api = (
        "https://financialmodelingprep.com/api/v3/financials/income-statement/" + fincompanyname + "?period=quarter"
    )
    balance_sheet_annual = (
        "https://financialmodelingprep.com/api/v3/financials/balance-sheet-statement/" + fincompanyname
    )
    ...
    cashflow_quarterly = (
        "https://financialmodelingprep.com/api/v3/financials/cash-flow-statement/" + fincompanyname + "?period=quarter"
    )
    ...
    return render_template(
        "financestate.html",
        companynames=fincompanyname,
        finance_annual=finance_data,
        finance_quarter=finance_quarter_data,
        balancesheetannual=balance_sheet_annual_data,
        balancesheetquarter=balance_sheet_quarterly_data,
        cashflowannual=cashflow_annual_data,
        cashflowquarter=cashflow_quarterly_data,
    )
...
def longterm_search():
    ...
    # print(mycursor.rowcount)
    return render_template(
        "longtermsearch.html", compname=comname, comprice=compprice, comexchange=comexchange, compsymbol=comsymbol
    )
...
def intraday_search():
    ...
    # print(mycursor.rowcount)
    return render_template(
        "intradaysearch.html", compname=comname, comprice=compprice, comexchange=comexchange, compsymbol=comsymbol
    )
...
def intraday_prediction(intracomp):
    ...
    return render_template(
        "intra.html",
        companyname=compnames,
        newsapi=news_result,
        twitterapi=twitter_result,
        times=new_minutes,
        intratest=intratestreturn,
        pricelist=price_list,
        predictions=returndata[0],
        accuracy=returndata[1],
    )
